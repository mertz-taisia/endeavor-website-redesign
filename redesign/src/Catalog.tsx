...
const endIndex = Math.min(items.length, startIndex + visibleItems);
  ...
  const endeavorVariants = {
    ...
    pullOutItem: { opacity: 1, x: 480, y: 400, transition: { duration: 0.6, ease: "easeOut" } }
  };
          ...
          {items.map((item, index) => {
            ...
            return (
              <motion.g
                ...
                animate={{ 
                  opacity: 1,
                  y: centerY, // Stay at center position
                  x: -305, // Not as far to the left
                  scale: 1.1, // Slightly larger as it moves out
